...
resource "proxmox_virtual_environment_vm" "gh_runner" {
  ...
  # which propagates into user_data_file_id above and forces the VM to be
  # replaced. Going from 2 workers to 3 rebuilt gunner-0 and gunner-1 as
  # collateral, killing whatever they were running. It is the same cascade that
  # destroyed the production cmd_and_ctrl VM and its data disk on 2026-09-10 --
  # runners are stateless so the stakes are lower here, but it still cancels
  # in-flight jobs and is never what "add a runner" is meant to do.
  #
